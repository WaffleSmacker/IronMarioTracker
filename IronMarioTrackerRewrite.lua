...
local function get_song_name(song_id)
    ...
    if song_info then
        return song_info[1] .. " - " .. song_info[2]
    end
    return "Unknown"
end
...
local function render_ui()
    ...
    gui.drawImage("ironmario_tracker/logo.png", game_width, game_height - (icon_size + 20), icon_size, icon_size)
    ...
    for i, entry in ipairs(star_entries) do
        ...
        for j = 1, entry.count do
            gui.drawImage("ironmario_tracker/star.png", icons_start_x + (j - 1) * font_size, y + (font_size * 0.1),
